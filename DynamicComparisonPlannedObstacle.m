...
bigObstacle=input('Komplette Blockade(1) oder kleines Hindernis(0)? ');
...
for k=1:numIterations
    ...
    %%
        ...
        startValid=false;
        while ~goalValid
            goal=[mapsize-6.8+(6)*rand(1) 19+1.5*rand(1) 0]; %start and goal are located within a smaller area on the top of the map so the initial path is always trough the top cooridor;start is on the left side and goal on the right side
            goalValid=stateValidatorBinary(goal,map_binary,res);
        end
        
        while ~startValid
            start=[0.8+6*rand(1) 19+1.5*rand(1) 0];
...
%         map.show
    ...
    pathQT=A_Star(nodesQT,start,goal,map_binary,res,validationDistance);
    timeQT(k,1)=toc;
    ...
    %%
    %Add an obstacle in the middle of the path depending on the size chosen
    %at the start
    %Blockade: completely block the upper corridor
    %small obstacle: only block a portion of the upper corridor
    ...
    if ~isempty(pathQT)&&~isempty(pathFQTDStar)&&~isempty(pathRRTX)&&~isempty(pathRRTStar)&&~isempty(pathQTDStar)
       if bigObstacle==1 %block completely
        map_binary(49:70,144:148)=1;
        map=occupancyMap(map_binary,res);
        sv.Map=map;
       else
        map_binary(49:80,132:134)=1;
        map=occupancyMap(map_binary,res);
        sv.Map=map;
       end
    else
    ...
    if isempty(pathQTDStar)
        ...
        % plot(pathQTDSTar(:,1),pathQTDStar(:,2),'-rx','LineWidth',1)
    end
...
cd 'Ergebnisse'
save(['DynamicComparisonPlannedObstacle_Iterations_',num2str(numIterations),'_',date,'bigObstacle_',num2str(bigObstacle)])