...
path_found=false;
openList(1,1:4)=[goalAdress-1, NaN,0, calculateHeuristic(tree.states(goalAdress-1,:),goal)];  %temp-1 is the adress of the start because it is added at the end
openList(1,5)=openList(1,4);

%this part is for creating a animation of A* searching a path

frame=0;
map=occupancyMap(mapBinary,mapRes);
figure1=figure;
hold on
map.show

plotEdges(tree.states,tree.neighbours)
plot(tree.states(:,1),tree.states(:,2),'or')
plot(goal(1),goal(2),'gs','LineWidth',2)
...
while size(openList)>0
    ...
    %mark all visited nodes(optional) --> this is also part of the
    %animation
    frame=frame+1;
    plot(tree.states(currentNode(1),1),tree.states(currentNode(1),2),'rs','LineWidth',2)
    saveas(figure1,['frame',num2str(frame),'.jpg'])
%     
